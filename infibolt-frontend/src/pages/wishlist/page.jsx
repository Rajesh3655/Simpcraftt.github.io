...
export default function WishlistPage() {
  ...
  return (
    <CommerceShell eyebrow="Wishlist" title="Saved products for later." description="A quiet place for the products you want to return to.">
      <MotionSection className="pb-10 md:pb-16 lg:pb-20">
        <div className="mx-auto w-full max-w-[1400px] space-y-10 px-6 md:px-8 lg:px-12">
          {wishlist.length === 0 ? <EmptyState title="No saved products yet" description="Save a product to keep it close for your next visit." action={<Link to="/products" className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-5 py-3 text-xs font-bold uppercase tracking-[0.14em] text-white"><Heart className="h-4 w-4" /> Explore products</Link>} /> : <ProductGrid items={wishlist} />}
