...
export default function CartPage() {
  ...
  return (
    <CommerceShell eyebrow="Checkout Flow" title="Cart" description="Direct checkout is API-ready with empty, populated, and future payment states prepared.">
      <MotionSection className="pb-10 md:pb-16 lg:pb-20">
        <div className="mx-auto w-full max-w-[1400px] space-y-10 px-6 md:px-8 lg:px-12">
          {cart.length === 0 ? <EmptyState title="Your cart is empty" description="Direct checkout will activate after backend order services are connected. You can still explore products and save purchase intent." action={<Link to="/products" className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-5 py-3 text-xs font-bold uppercase tracking-[0.14em] text-white"><ShoppingCart className="h-4 w-4" /> Browse products</Link>} /> : <ProductGrid items={cart} />}
